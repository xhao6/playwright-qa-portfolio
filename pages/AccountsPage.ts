...
// open() 列表就绪探测：最多 10 次 × 3s，覆盖编辑保存后的表单 teardown 弹窗与 SPA 渲染
const OPEN_RETRY_LIMIT = 10;
const OPEN_PROBE_TIMEOUT_MS = 3_000;
// 保存后跳详情：服务端写入 + 表单 teardown，实测可达 ~10s，预算 30s
...
export class AccountsPage {
  ...
  get editButton() {
    return this.page.locator('button[data-action="edit"]');
  }
  ...
  async create(data: AccountData) {
    ...
    await expect(this.page).toHaveURL(/#\/?Account\/view\//, { timeout: SAVE_TIMEOUT_MS });
  ...
  nameCellLink(name: string) {
    return this.rowByName(name).locator('td[data-name="name"] a.link');
  }

  async openRecord(name: string) {
    await this.nameCellLink(name).click({ timeout: CLICK_TIMEOUT_MS });
    await expect(this.detailNameField).toBeVisible();
  }
  ...
  async removeByName(name: string) {
    ...
    for (let attempt = 0; attempt < REMOVE_RETRY_LIMIT; attempt++) {
      try {
        ...
        // probe 实测删除 XHR 为 DELETE /api/v1/Account/<id>（响应 200）；等其完成而非只等乐观移除
        const deleteResponse = this.page.waitForResponse(
          (r) => r.request().method() === 'DELETE' && r.url().includes('/api/v1/Account/'),
